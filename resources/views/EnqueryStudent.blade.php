...
<style>
  ...
  .border-on-hover:hover {
    ...
    /* Change the border color on hover to blue with !important */
  }
</style>
...
<button class="btn btn-default" id="filter" type="button"><i class="las la-filter font-17"></i>
  Filter</button>&nbsp;&nbsp;
...
<div class="widget-content widget-content-area p-0">
  ...
  <table id="list" class="table table-bordered mb-0 p-0 mt-2" style="width: 100%;">
    <thead>
      <tr>
        <th>
          <div id="filterform" class="p-0 m-0" style="display:none;" class="ml-3 mr-3 mb-1  p-1  mt-2 bg-light"
            style="border-left: 5px solid #f3a129">
            <form class="row col-md-12 m-0 p-0  mt-2" action="{{ route('enq.filter') }}" method="GET">
              <input type="date" name="start_date" class="form-control col-md-2 ml-1"
                style="font-size: 14px; padding: 0.25rem 0.5rem;height: 35px;">
              <input type="date" name="end_date" class="form-control custom_input col-md-2 ml-1"
                style="font-size: 14px; padding: 0.25rem 0.5rem;height: 35px;">
              <select class="form-control custom_input col-md-2 ml-1" name="date_filter"
                style="font-size: 14px; padding: 0.25rem 0.5rem;height: 35px;">
              ...
              <select class="form-control custom_input col-md-2 ml-1" name="verified_filter"
                style="font-size: 14px; padding: 0.25rem 0.5rem;height: 35px;">
                <option value="verified">Verified</option>
                <option value="not_verified">Not Verified</option>
                <option value="all">All</option>
              </select>
              <button class="btn btn-dark form-control col-md-1 ml-1" type="submit"
                style="font-size: 14px; padding: 0.25rem 0.5rem;height: 35px;"><i
                  class="las la-search font-20"></i></button>

              <input type="text" id="tblSearch" placeholder="Search by Id , Name etc"
                class="form-control custom_input col-md-2 ml-1"
                style="font-size: 14px; padding: 0.25rem 0.5rem;height: 35px;">
            </form>
          </div>
        </th>
            ...
            <div class="single-result row m-0 p-2">
              ...
              <div class="col-md-3">
                <div class="d-flex align-items-center mb-1 mr-3">
                  ...
                  <a class="text-warning">Verified by @if(Auth::user()->id == $row->verified_by) You @else {{
                    $row->verify_user->name }} @endif</a>
                  @else
              ...
              <div class="col-md-3 buy-now-area">
                ...
                <div class="d-flex align-items-center mt-4 mb-2 ml-3">
                  <button type="submit" class="btn btn-default  "
                    style="font-size: 30px; padding: 0.25rem 0.5rem;height: 36px;"><i
                      class="lab la-whatsapp font-16 text-success "></i></button>
                  <button class="btn btn-defalt" style="font-size: 30px; padding: 0.25rem 0.5rem;height: 36px;"
                    id="emailsend" type="button" data-name="{{ $row->name }}" data-email="{{ $row->email }}"
                    data-image="{{ (!empty($row->photo)) ? asset('uploads/'.$row->photo.''):asset('assets/assets/img/NoProfile.png') }}"><i
                      class="las la-envelope font-16  text-primary "></i></button>
                  <a class="enq-del-con" data-id="{{ route('enq.delete', $row->id) }}" href="#"><button type="submit"
                      class="btn btn-default " style="font-size: 30px; padding: 0.25rem 0.5rem;height: 36px;"><i
                        class="las la-trash font-16 text-danger "></i></button></a>
                </div>
              ...
              <div class="row">
                ...
                <div class="single-result p-0 ml-1">
                  <div class="col-md-1 d-flex">
                    <div class="image-container ">
                      ...
                      @else
                      <a class="admin-vrify-confirm"
                        data-id="{{ route('verify.stu.enq', [$row->id, Auth::user()->id ] ) }}" href="#"><i
                          class="las la-check-circle font-35 text-danger"></i></a>
                      @endif
                    </div>
                  </div>
                </div>
                ...
                <div class="col-md-5 p-0">
                  <div class="d-flex align-items-center ">
                    <i class="las la-school font-25 text-secodary"></i></a> &nbsp;<p class="font-13 mt-2">Interest : {{
                      $row->remarks }}...</p>
                  </div>
                  <div class="d-flex align-items-center ">
                ...
                <div class="col-md-3 p-0">
                  <div class="d-flex align-items-center ">
                    <i class="las la-clock font-20 text-secodary"></i></a> &nbsp;<p class="font-13 mt-2">{{ date('F j,
                      Y', strtotime($row->created_at)) }}</p>
                  </div>
                  <div class="d-flex align-items-center ">
                ...
                <div class="col-md-3 float-end p-0">
                  <form method="post" action="{{ route('verify.stu') }}">
                    ...
                    <div class="d-flex">
                      <select class="form-control mt-4 ml-0" required="" name="users"
                        style=" font-size: 14px; padding: 0.25rem 0.5rem;height: 35px;">
                        <option value="">Assign student</option>
                        @foreach ($users as $rows)
                        @foreach($rows->roles->pluck('name') as $role)
                        <option value="{{ $rows->id }}">{{ $rows->name }} ({{ $role }} )@if(Auth::user()->id ===
                          $rows->id ) My self @endif</option>
                        @endforeach
                        @endforeach
                      </select>
                      <button type="submit" class="btn btn-default mt-4"
                        style="font-size: 18px; padding: 0.25rem 0.5rem;height: 35px;"><i
                          class="las la-caret-square-right font-18 text-warning"></i></button>
                    </div>
                  </form>
                </div>
                <!----------- --------->
          ...
          <div class="widget widget-content p-2 m-1">
            <div class="single-result row m-0 p-0">
              ...
              <div class="col-md-2 buy-now-area">
                ...
                @else
                <a class="user-vrify-confirm" data-id="{{ route('verify.stu.enq', [$row->id, Auth::user()->id ] ) }}"
                  href="#"><i class="las la-certificate font-45 mt-2"></i></a>
      ...
      <form method="POST" action="{{ route('store.stu') }}" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <div class="col-md-12 row">
            <div class="from-control col-md-6">
              ...
              <input type="text" placeholder="Enter student Name" name="name" value="{{ old('name') }}"
                class="form-control">
              @error('name')<ul class="parsley-errors-list filled text-danger" id="parsley-id-13" aria-hidden="false">
                <li class="">{{ $message }}</li>
              </ul>@enderror
            </div>
            ...
            <div class="from-control col-md-6">
              ...
              <input type="email" name="email" value="{{ old('email') }}" placeholder="Enter student Email address"
                class="form-control">
          ...
          <div class="col-md-12 row">
            ...
            <div class="from-control col-md-6">
              ...
              @error('confirm_password')<ul class="parsley-errors-list filled" id="parsley-id-13" aria-hidden="false">
                <li class="parsley-required text-danger">{{ $message }}</li>
              </ul>@enderror
            </div>
          </div><br>
          ...
          <div class="col-md-12 row">
            ...
            <div class="from-control col-md-6">
              ...
              <div class="col-sm-10">
                <img class="rounded " alt="100x100" width="120" src="{{ asset('assets/assets/img/NoProfile.png') }}"
        ...
        <div class="modal-footer">
          <div class="row col-md-12">
            ...
            <div class="col-md-4">
              <button type="submit" class="btn btn-outline-primary form-control waves-effect waves-light float-end">Add
                Enquiry</button>
            </div>

            <div class="col-md-4">
              <button type="button" class="btn btn-md btn-outline-dark form-control float-end  waves-effect waves-light"
